...
class CreatePasswordAccountScreen extends StatefulWidget {
  final String? phoneNumber;
  final bool isRegister;
  const CreatePasswordAccountScreen({Key? key, this.phoneNumber, required this.isRegister}) : super(key: key);

  @override
  _CreatePasswordAccountScreenState createState() =>
      _CreatePasswordAccountScreenState();
}

class _CreatePasswordAccountScreenState
    extends State<CreatePasswordAccountScreen> {
  ...
  Future<dynamic> registerFarmer(String phoneNumber, String password) async{
    var response = await _accountRepository.registerFarmer(phoneNumber, password);
    setState(() {
      if(response['status']['status code'] == 200){
        ...
        Navigator.push(context, PageTransition(
                ...
                child:  FillAccountInfoScreen(farmerId: response['data'],username: widget.phoneNumber.toString(),
                  password: password,),
                childCurrent: CreatePasswordAccountScreen(isRegister: widget.isRegister,)));
      }else{
        isCall = false;
                  ...
                  child: Form(
                    ...
                    child: Column(
                      children: [
                          ...
                          children: [
                            ...
                            Text('Tạo mật khẩu',
                                style: TextStyle(fontFamily: 'BeVietnamPro', fontWeight: FontWeight.w600,
                                    fontSize: 16.sp, color: Color.fromRGBO(0, 0, 0, 1.0))),
                          ],
                        ),
                        ...
                        SizedBox(height: _size.height * 0.055,),
                        Container(
