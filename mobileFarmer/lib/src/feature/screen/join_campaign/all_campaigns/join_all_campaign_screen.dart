...
import 'components/list_all_campaigns.dart';
import 'join_all_campaign_bloc.dart';
import 'package:http/http.dart' as http;

class JoinAllCampaignScreen extends StatefulWidget {
  const JoinAllCampaignScreen({Key? key}) : super(key: key);

  @override
  _JoinAllCampaignScreenState createState() => _JoinAllCampaignScreenState();
}

class _JoinAllCampaignScreenState extends State<JoinAllCampaignScreen> {
  ...
  Future getCountCampaign(String farmerId) async {
    final campaigns = await _campaignRepository.getCountCampaignCanApplyByFarmer(farmerId, '');
    if (mounted) {
      setState(() {
    ...
    if (farmerId != '') {
      return Expanded(
          child: SingleChildScrollView(
            child: BlocProvider(
              create: (_) => JoinAllCampaignBloc(httpClient: http.Client(), farmerId: farmerId)
                ..add(AllCampaignFetched()),
              child: ListAllCampaigns(farmerId: farmerId,),
            ),
          ));
    }
    return Container(
