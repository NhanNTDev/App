...
import 'components/list_event_campaigns.dart';
import 'join_event_campaign_bloc.dart';
import 'package:http/http.dart' as http;

class JoinEventCampaignScreen extends StatefulWidget {
  const JoinEventCampaignScreen({Key? key}) : super(key: key);

  @override
  _JoinEventCampaignScreenState createState() => _JoinEventCampaignScreenState();
}

class _JoinEventCampaignScreenState extends State<JoinEventCampaignScreen> {
  ...
  Future getCountCampaign(String farmerId) async {
    final campaigns = await _campaignRepository.getCountCampaignCanApplyByFarmer(farmerId, 'event');
    if (mounted) {
      setState(() {
    ...
    if (farmerId != '') {
      return Expanded(
          child: SingleChildScrollView(
            child: BlocProvider(
              create: (_) => JoinEventCampaignBloc(httpClient: http.Client(), farmerId: farmerId)
                ..add(EventCampaignFetched()),
              child: ListEventCampaigns(farmerId: farmerId,),
            ),
          ));
    }
    return Container(
                        ...
                        onPressed: () {
                          Navigator.push(
                              ...
                              PageTransition(curve: Curves.easeInOut,
                                  ...
                                  childCurrent: const JoinCampaignScreen(initPage: 1,)));
                        },
                        child: IconWidget(
