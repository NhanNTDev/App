...
import 'components/list_cancel_orders.dart';
import 'package:http/http.dart' as http;

class CancelFarmOrderScreen extends StatefulWidget {
  const CancelFarmOrderScreen({Key? key}) : super(key: key);

  @override
  _CancelFarmOrderScreenState createState() => _CancelFarmOrderScreenState();
}

class _CancelFarmOrderScreenState extends State<CancelFarmOrderScreen> {
        ...
        if (entry.key == 'userId')
          {
            ...
            if (farmerId != '') {getCountFarmOrder(farmerId,7)}
          }
      })
    ...
    if (farmerId != '') {
      return Expanded(
          child: SingleChildScrollView(
              child: BlocProvider(
                create: (_) =>
                CancelFarmOrderBloc(httpClient: http.Client(), farmerId: farmerId)
                  ..add(CancelFarmOrderFetched()),
                child: const ListCancelFarmOrder(),
              )
          ));
    }
