...
import 'complete_order_card.dart';

class ListCompleteFarmOrder extends StatefulWidget {
  const ListCompleteFarmOrder({Key? key}) : super(key: key);

  @override
  _ListCompleteFarmOrderState createState() => _ListCompleteFarmOrderState();
}

class _ListCompleteFarmOrderState extends State<ListCompleteFarmOrder>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<CompleteFarmOrderBloc, CompleteFarmOrderState>(
      builder: (context, state) {
        switch (state.status) {
          case CompleteFarmOrderStatus.failure:
            return const Center(child: Text('Đã có lỗi xảy ra'));
          case CompleteFarmOrderStatus.success:
            if (state.farmOrders.isEmpty) {
              return const Center(child: Text('Hiện chưa có đơn hàng hoàn thành'));
            }
            return SizedBox(
                                  ...
                                  setState(() {
                                    Navigator.push(
                                        ...
                                        PageTransition(
                                            ...
                                            child: CompleteOrderDetailScreen(
  ...
  void _onScroll() {
    if (_isBottom) context.read<CompleteFarmOrderBloc>().add(CompleteFarmOrderFetched());
  }
