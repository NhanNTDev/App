...
import '../need_confirm_order_screen.dart';
import 'need_confirm_order_card.dart';

class ListNeedConfirmFarmOrder extends StatefulWidget {
  const ListNeedConfirmFarmOrder({Key? key}) : super(key: key);

  @override
  _ListNeedConfirmFarmOrderState createState() => _ListNeedConfirmFarmOrderState();
}

class _ListNeedConfirmFarmOrderState extends State<ListNeedConfirmFarmOrder>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<NeedConfirmFarmOrderBloc, NeedConfirmFarmOrderState>(
      builder: (context, state) {
        switch (state.status) {
          case NeedConfirmFarmOrderStatus.failure:
            return const Center(child: Text('Đã có lỗi xảy ra'));
          case NeedConfirmFarmOrderStatus.success:
            if (state.farmOrders.isEmpty) {
              return const Center(child: Text('Hiện không có đơn hàng cần xác nhận'));
            }
            return SizedBox(
                                  ...
                                  setState(() {
                                    Navigator.push(
                                        ...
                                        PageTransition(
                                            ...
                                            child: NeedConfirmOrderDetailScreen(
  ...
  void _onScroll() {
    if (_isBottom) context.read<NeedConfirmFarmOrderBloc>().add(NeedConfirmFarmOrderFetched());
  }
