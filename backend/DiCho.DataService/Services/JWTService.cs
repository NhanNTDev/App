...
namespace DiCho.DataService.Services
{
    ...
    {
        Task<string> CreateUser(AspNetUsersCreateModel model);
        ...
        Task<List<CustomerUserModel>> GetUserCustomer(int? zoneId, string customerId);
        Task<CustomerUserModel> GetUserIdOfPublish(string id);
        Task<IdentityResult> UpdatePublished(string id, AspNetUserUpdatePublisedModel modelInput);
    }
    public class JWTService : IJWTService
    {
        ...
        public async Task<string> CreateUser(AspNetUsersCreateModel model)
        {
            ...
            var user = new AspNetUsers
            {
                ...
                Active = true,
                Image = "https://firebasestorage.googleapis.com/v0/b/dichonao-f5871.appspot.com/o/Images%2FAvatar%2FnamDefault.jpg?alt=media&token=2b7393ff-e3d5-443a-bbde-f92970425ff9"
            };
            if (model.Role.FirstOrDefault().Name == "customer")
        ...
        public async Task<IdentityResult> UpdateUser(string id, AspNetUsersUpdateModel modelInput)
        {
            ...
            if (entity.AspNetUserRoles.Any(x => x.Role.Name == "customer"))
            {
                int zone = 0;
                if (modelInput.Address != null)
                {
                    modelInput.Address = await _tradeZoneMapService.GetAddress(modelInput.Address);
                    zone = _tradeZoneMapService.CheckZone(modelInput.Address).Result.f4;
                }
                updateEntity.ZoneId = zone;
            }
        ...
        public async Task<TokenModel> Login(AspNetUserLoginModel model)
        {
            ...
            if (!user.Active)
                throw new ErrorResponse((int)HttpStatusCode.BadRequest, "Tài khoản đã bị khóa!");
        ...
        public async Task<List<CustomerUserModel>> GetUserCustomer(int? zoneId, string customerId)
        {
            var customers = await _userManager.Users.Where(x => x.ZoneId == zoneId && x.Id != customerId && x.AspNetUserRoles.Any(y => y.Role.Name == "customer")).ProjectTo<CustomerUserModel>(_mapper).ToListAsync();
            return customers;
        }

        public async Task<CustomerUserModel> GetUserIdOfPublish(string id)
        {
            var user = await _userManager.FindByIdAsync(id);
            if (user == null)
                throw new ErrorResponse((int)HttpStatusCode.NotFound, $"Không tìm thấy");
        ...
        public async Task<IdentityResult> UpdatePublished(string id, AspNetUserUpdatePublisedModel modelInput)
        {
            var entity = await _userManager.FindByIdAsync(id);
            if (modelInput.Id != id || entity == null)
                throw new ErrorResponse((int)HttpStatusCode.NotFound, $"Không tìm thấy!!");
            ...
            int zone = 0;
            if (modelInput.Address != null && entity.Address != modelInput.Address)
            {
                modelInput.Address = await _tradeZoneMapService.GetAddress(modelInput.Address);
                zone = _tradeZoneMapService.CheckZoneForCampagin(modelInput.Address).Result;
            }
            var model = new AspNetUserUpdatePublisedModel
            {
                Address = modelInput.Address,
                DateOfBirth = modelInput.DateOfBirth,
                Email = modelInput.Email,
