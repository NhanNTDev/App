...
namespace DiCho.API.Controllers
{
    ...
    public class ExternalsController : ControllerBase
    {
        ...
        [HttpGet("zone-delivery")]
        [MapToApiVersion("1")]
        public async Task<IActionResult> GetZonesForCampaignDeliveryZone()
        {
            var result = await _tradeZoneMapService.GetZonesForCampaignDeliveryZone();
            return Ok(result);
        }
        /// <summary>
        /// get zone by address
        ...
        /// <returns></returns>
        //[Authorize(Roles = "admin")]
        [HttpGet("zone-campaign")]
        [MapToApiVersion("1")]
        public async Task<IActionResult> GetZonesForCampaignZone()
        {
            var result = await _tradeZoneMapService.GetZonesForCampaignZone();
            return Ok(result);
        }
