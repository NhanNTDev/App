import { Link } from "react-router-dom";

const FarmGroup = (props) => {
  function renderCampaignItem(props) {
    return (
      <div className="item" key={props.id}>
        <div className="product">
          <Link to={`/campaign/${props.id}`}>
            <div className="product-header">
              <span className="badge badge-success">50% OFF</span>
              <img className="img-fluid" src={props.image} alt="" />
              <span className="veg text-success mdi mdi-circle"></span>
            </div>
            <div className="product-body">
              <h4 style={{ height: 80 }}>{props.name}</h4>
              <h5>
                <strong>
                  <span className="mdi mdi-flower"></span> Địa chỉ:
                </strong>{" "}
                {props.location}
              </h5>
              <h5>
                <strong>
                  <span className="mdi mdi-map-marker-circle"></span> Khu vực:
                </strong>{" "}
                {props.area}
              </h5>
              <h5>
                <strong>
                  <span className="mdi mdi-account"></span> Chủ nông trại:
                </strong>{" "}
                {props.owner}
              </h5>
              <br />
            </div>
          </Link>
          <div className="product-footer">
            <button
              type="button"
              className="btn btn-secondary btn-sm float-right"
              onClick={() => {}}
            >
              <i className="mdi mdi-eye"></i> Xem Chi Tiết
            </button>
            <br />
          </div>
        </div>
      </div>
    );
  }

  return (
      <section className="product-items-slider section-padding">
      <div className="container">
        <div className="section-header">
          <h5 className="heading-design-h5">
            {props.title}{" "}
            {props.type === "hot" && (
              <span className="badge badge-primary">Đang hot</span>
            )}
            <Link
              className="float-right text-secondary"
              to={`/all-campaigns?type=${props.type}`}
            >
              Xem tất cả
            </Link>
          </h5>
        </div>
        <div className="owl-carousel owl-carousel-featured">
          {props.listFarms &&
            props.listFarms.map((farm) =>
              renderCampaignItem({ ...farm })
            )}
        </div>
      </div>
    </section>
  );
};

export default FarmGroup;
