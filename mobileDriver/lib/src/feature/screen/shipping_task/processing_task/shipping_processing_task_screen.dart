...
import 'components/list_shipping_processing_tasks.dart';

class ShippingProcessingTaskScreen extends StatefulWidget {
  const ShippingProcessingTaskScreen({Key? key}) : super(key: key);

  @override
  _ShippingProcessingTaskScreenState createState() => _ShippingProcessingTaskScreenState();
}

class _ShippingProcessingTaskScreenState extends State<ShippingProcessingTaskScreen> {
    ...
    if (deliveryDriverId != '') {
      return Expanded(
          child: SingleChildScrollView(
              child: BlocProvider(
                create: (_) =>
                ShippingProcessingTaskBloc(httpClient: http.Client(), driverId: deliveryDriverId)
                  ..add(ShippingProcessingFetched()),
                child: ListShippingProcessingTask(deliveryDriverId: deliveryDriverId,),
              )
          ));
    }
    return Container(
