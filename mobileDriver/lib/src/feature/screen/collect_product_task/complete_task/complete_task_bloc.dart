...
class CompleteTaskBloc
    extends Bloc<CompleteTaskEvent, CompleteTaskState> {
  final String? search;
  final String farmerId;

  CompleteTaskBloc({required this.httpClient, this.search, required this.farmerId})
      : super(const CompleteTaskState()) {
    on<CompleteFetched>(
      _onCompleteTaskFetched,
  ...
  Future<void> _onCompleteTaskFetched(
      CompleteFetched event,
      Emitter<CompleteTaskState> emit,
      ) async {
    if (state.hasReachedMax) return;
    try {
      if (state.status == CompleteTaskStatus.initial) {
        if(state.collectOrders.isNotEmpty){
          state.collectOrders.clear();
        }
        // final collectOrders = await _collectOrderRepository.getListFarmOrder(i, _collectOrderLimit,farmerId, 4);
        final collectOrders = await _collectOrderRepository.getListFarmOrder(i, _collectOrderLimit,farmerId, true);
        return emit(state.copyWith(
          status: CompleteTaskStatus.success,
      ...
      emit(state.copyWith(status: CompleteTaskStatus.failure));
    }
  }
}
